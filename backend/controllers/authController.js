...
const toggleUserStatus = async (req, res) => {
  ...
  try {
    ...
    if (req.user.id === id) return res.status(400).json({ message: 'Cannot deactivate your own account' });
    user.isActive = !user.isActive;
    await user.save();
    res.status(200).json({ message: `User ${user.isActive ? 'activated' : 'deactivated'} successfully`, user: { ...user.toObject(), password: undefined } });
  } catch (err) {
    res.status(500).json({ message: 'Server error' });
  }
};

module.exports = { login, getUsers, createUser, updateUser, deleteUser, changePassword, getProfile, updateProfile, forgotPassword, resetPassword, toggleUserStatus };
